...
- [ ] AWS Pricing MCP configured (`awslabs.aws-pricing-mcp-server`, region: us-east-2)
...
- [ ] Script accepts region as parameter (default: us-east-2)
...
- [ ] `variables.tf` defines aws_region variable (default: us-east-2)
...
**Priority:** P0
**Description:** Create ECR private repositories for all 8 microservices with lifecycle policies, scan-on-push, and configurable tag immutability (MUTABLE dev, IMMUTABLE prod). Images stored at `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service}:{tag}`. Cost: ~$1/month beyond 500 MB free tier.
**Blocked by:** E-1
**Blocks:** E-10, E-17
...
- [ ] `terraform apply` succeeds
- [ ] 8 ECR repositories visible in us-east-2 under `petclinic-dev/` prefix
...
**Description:**
Create `scripts/ecr-login.sh` that authenticates Docker to the ECR private registry in us-east-2.

**Technical Spec:** [ECR Container Registry](./technical-spec.md#ecr-container-registry)

**Acceptance Criteria:**
- [ ] Script at `scripts/ecr-login.sh`
- [ ] Uses `aws ecr get-login-password --region us-east-2` and pipes to `docker login 428101261622.dkr.ecr.us-east-2.amazonaws.com`
- [ ] Works on macOS and Linux
- [ ] Accepts optional `--region` parameter (defaults to us-east-2)
...
- [ ] Image: `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/config-server:<TAG>` (placeholder)
...
- [ ] Push to ECR: `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-dev/{service}:{sha}`
...
- [ ] ECR login successful: `aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin 428101261622.dkr.ecr.us-east-2.amazonaws.com`
- [ ] All 8 images tagged with initial version (e.g., `v1.0.0` or commit SHA)
- [ ] All 8 images pushed to ECR (`428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-dev/{service}:{tag}`)
...
- [ ] GDPR considerations: us-east-2 data residency noted
...
- [ ] Each file specifies: image repo (`428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service}`), image tag, container port, service port
