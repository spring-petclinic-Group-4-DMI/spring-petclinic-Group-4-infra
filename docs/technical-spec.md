...
>
> **Scope (current):** **Only the dev environment is deployed.** The prod environment has been removed from this project — any "Prod" columns and `petclinic-prod` references in tables below are historical and do **not** apply. Treat the dev column as canonical. The prod terraform environment under `terraform/environments/prod/` has been deleted; no prod ArgoCD Applications, no prod ECR repos, no prod state file.
...
| S3 Bucket | `petclinic-terraform-state-428101261622` |
...
- Accepts `--region` parameter (default: `us-east-2`)
...
| Region | `us-east-2` (same as infra) | `us-east-2` |
...
|-----------------|---------|-------------------|
| `petclinic-{env}/config-server` | Config Server | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/config-server:{tag}` |
| `petclinic-{env}/discovery-server` | Discovery Server | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/discovery-server:{tag}` |
| `petclinic-{env}/api-gateway` | API Gateway | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/api-gateway:{tag}` |
| `petclinic-{env}/customers-service` | Customers Service | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/customers-service:{tag}` |
| `petclinic-{env}/visits-service` | Visits Service | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/visits-service:{tag}` |
| `petclinic-{env}/vets-service` | Vets Service | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/vets-service:{tag}` |
| `petclinic-{env}/genai-service` | GenAI Service | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/genai-service:{tag}` |
| `petclinic-{env}/admin-server` | Admin Server | `428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/admin-server:{tag}` |

### ECR Authentication

```bash
# Login (same region as infra)
aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin 428101261622.dkr.ecr.us-east-2.amazonaws.com

# Push
docker push 428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service}:{tag}
```
...
Example: `jdbc:mysql://petclinic-dev-mysql.abc123.us-east-2.rds.amazonaws.com:3306/petclinic`
...
5. Login to ECR: `aws ecr get-login-password --region us-east-2`
...
|-------------------|--------------------|-----------|------------|---------|
| `petclinic-{env}-eso-role` | `external-secrets-sa` | `external-secrets` | `secretsmanager:GetSecretValue`, `secretsmanager:DescribeSecret` on `arn:aws:secretsmanager:us-east-2:428101261622:secret:petclinic/*` | ESO |
...
```json
{
  ...
  "Statement": [{
    "Effect": "Allow",
    "Principal": {
      "Federated": "arn:aws:iam::428101261622:oidc-provider/{oidc-provider}"
    },
...
replicaCount: 1
image:
  repository: ""   # Set per-service: 428101261622.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service}
...
spec:
  project: default
  source:
    repoURL: https://github.com/{your-username}/spring-petclinic-Group-4-infra.git
    targetRevision: main
